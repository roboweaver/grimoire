...
- [x] 0.3 Open the spec PR and obtain review approval before writing code.
  - _Acceptance:_ PR open, CI green, approved.

## Phase 1 — `internal/routing` archive grammar (pure, no DB, no HTTP)

- [x] 1.1 Write failing table-driven tests for `routing.DateRef.Range`: year-only,
...
- [x] 1.2 Implement `DateRef` and `Range` in `internal/routing/routing.go`,
      ...
      _(Req 6.4)_
- [x] 1.3 Write failing tests for `Parse` recording base **provision**, not just
...
- [x] 1.4 Implement `CategoryBaseSet`, `TagBaseSet` and the derived front in
...
- [x] 1.5 Write failing tests for the four archive path constructors
...
- [x] 1.6 Write failing tests for `DatePath`'s `date/` disambiguation prefix over
...
- [x] 1.7 Implement the four constructors next to `Canonical`, so archive URLs
...
- [x] 1.8 Write a failing test for the **canonical fixed-point property** of each
      ...
      _(Req 2.4, 4.7, 4.8)_
- [x] 1.9 Write failing table-driven tests for `Structure.Classify` covering
...
- [x] 1.10 Implement `Kind`, `Target`, `AuthorBase` and `Classify`. `Classify` is
...
- [x] 1.11 Write failing tests for `Structure.ArchivePatterns`: the expected chi
...
- [x] 1.12 Implement `ArchivePatterns`. _(Req 9.5)_
- [x] 1.13 Write a failing test for base collisions: `category_base` equal to
      ...
      **nil error** and a fully usable non-flat `Structure` whose colliding
      ...
      _(Req 4.4, 4.5)_
- [x] 1.14 Implement `Structure.Notes()` and the collision check in `Parse`.
      Degrade loudly but keep serving, the same posture M9a Requirement 4 takes.
      _(Req 4.4, 4.5)_

## Phase 2 — `Term.ParentID` read path (cross-vendor)

- [x] 2.1 Extend `storagetest.SeedFixtures` with a nested category chain —
      ...
      _(Req 12.1)_
- [x] 2.2 Write failing `storagetest` contract cases asserting `ParentID` is
      ...
      _(Req 1.2, 12.1)_
- [x] 2.3 Add `ParentID int64` to `domain.Term` in
...
- [x] 2.4 Add `parent` to `termRow` and to all three term selects in
      `internal/storage/wprepo/repo.go`. Each already joins `term_taxonomy`, so
      this is one extra column and no new join. _(Req 1.2)_
- [x] 2.5 Confirm `term_taxonomy.parent` already exists in every vendor's
...
- [x] 3.1 Write failing `storagetest` contract cases for
...
- [x] 3.2 Add `ArchiveFilter` to `internal/domain/repository.go` following
...
- [x] 3.3 Implement both in `internal/storage/wprepo/repo.go` using an `EXISTS`
      ...
      _(Req 3.1, 3.3, 6.4)_
- [x] 3.4 Leave `ByTermSlug`/`CountPublishedByTermSlug` untouched. They are the
...
- [x] 4.1 Write a failing `storagetest` contract case for
...
- [x] 4.2 Add `ByNicename` to `domain.UserRepository` and implement it in
      ...
      _(Req 7.2, 7.6)_
- [x] 4.3 Write a failing `storagetest` contract case for
      ...
      _(Req 7.8, 12.10)_
- [x] 4.4 Add `domain.NicenameConflict` and
...
- [x] 5.1 Write failing tests for ancestry and descendant resolution against a
...
- [x] 5.2 Add `content.Archive` (in a new `internal/content/archive.go`) and
      ...
      _(Req 1.6)_
- [x] 5.3 Write failing tests for `TermService.CategoryArchive`: returns the
...
- [x] 5.4 Add `TermService.WithHierarchy(domain.TermReader)` and implement
...
- [x] 5.5 Write failing tests for `TermService.TagArchive`: flat resolution, no
      ...
      _(Req 5.1–5.4)_
- [x] 5.6 Implement `TagArchive`. _(Req 5.1, 5.2)_
- [x] 5.7 Remove `TermService.CategoryPage` and `TermService.Category`.
...
- [x] 5.8 Write failing tests for `PostService.AuthorArchive` and
...
- [x] 5.9 Add `PostService.WithAuthors(domain.UserRepository)` and implement
...
- [x] 6.1 Write a failing test asserting `render.ArchiveData` renders through the
...
- [x] 6.2 Add `ArchiveData` to `internal/render/view.go` and make `CategoryData`
...
- [x] 6.3 Write a failing test asserting `archive.tmpl` renders the supplied
      heading and, when `TotalPages > 1`, pagination links built from
      `.BaseURL`. _(Req 8.4, 11.3)_
- [x] 6.4 Update `themes/default/templates/archive.tmpl` to render `.Heading`
...
- [x] 7.1 Write failing handler tests covering **every row** of `design.md`'s
...
- [x] 7.2 Write a failing test asserting each archive kind's canonical path
...
- [x] 7.3 Write a failing test asserting the query string survives every archive
      canonical redirect. _(Req 2.9)_
- [x] 7.4 Write a failing **regression** test asserting the routes registered
...
- [x] 7.5 Add `Server.resolve` to `internal/web/handlers.go`: call
...
- [x] 7.6 In `internal/web/router.go`, replace the hard-coded
...
- [x] 7.7 Implement `categoryArchive`, `tagArchive`, `authorArchive` and
...
- [x] 7.8 Wire the new opt-in service dependencies in `cmd/grimoire/main.go`:
...
- [x] 8.1 Write a failing test asserting `restTerm.Parent` carries
...
- [x] 8.2 Write a failing test asserting `commentLink` is **unchanged** on its
      `/?p={id}#comment-{id}` fallback — no route is added for it, so changing it
      would advertise a URL that `404`s. _(Req 10.4)_
- [x] 8.3 Make `restTermLink` a method on `*Server` so it can reach the
...
- [x] 8.4 Point `userLink` in `internal/content/rest.go` at
      `Structure.AuthorPath`. _(Req 7.5, 10.3)_

## Phase 9 — Startup reporting

- [x] 9.1 Write a failing test asserting `grimoire-cli migrate -check` reports
      the resolved category and tag bases, and prints each `Structure.Notes()`
      entry when a base collides. _(Req 4.6)_
- [x] 9.2 Update `cmd/grimoire-cli/permalinks.go` to report both bases and the
...
- [x] 9.3 Confirm `cmd/grimoire/permalinks.go`'s startup `INFO` line still names
...
- [x] 9.4 Write a failing test asserting `grimoire-cli migrate -check` reports
...
- [x] 9.5 Wire `domain.NicenameAuditor.DuplicateNicenames` into `reportPreflight`
...
- [x] 10.1 Add a `test/e2e` test booting the stack with a nested category chain
      ...
      _(Req 12.5)_
- [x] 10.2 Extend `internal/routing/realdb_test.go` with the nested-category
...
- [x] 10.3 Extend `test/e2e/m9_permalinks_realdb_test.go` with the end-to-end
      ...
      _(Req 12.6)_
- [x] 10.4 Make both real-database extensions **skip** with a message naming the
...
- [x] 10.5 Run both env-gated checks against the live WordPress database — the
  ...
  - _Both env-gated checks have now passed against the live WordPress database._
    Target: the podman stack in `accuweaverllc/scripts` — MySQL 8.0, database
    `wordpress`, prefix `accuweaver`, `permalink_structure`
    `/%year%/%monthnum%/%day%/%postname%/` (so `trailing_slash=true` and an
    **empty front** — the structure leads with a token, not a literal) and
    `category_base` **empty**, which resolves to the default `category` and, per
    Req 4.8, is the *unset* case that would carry the front if there were one.
    MySQL is not published to the host, so both runs happened inside a
    `golang:1.26-alpine` container joined to the `wp-net` podman network with the
    repo mounted read-only and the DSN composed from the `wp-db-password` podman
    secret — the same route M9a's task 7.2 took.
  - _Observed on that database:_ **33 category terms, 28 with a resolvable
    `term_taxonomy.parent`, 28 yielding a usable nested path** — the census the
    introduction of `requirements.md` claims, confirmed rather than restated, and
    every nested term's whole ancestry is a plain URL token so none was skipped.
  - `TestRealWordPressNestedCategories` (task 10.2) passed — "validated 33 real
    category paths (28 nested) against base `category` and structure
    `/%year%/%monthnum%/%day%/%postname%/`". Every one of the 33 derived paths
    round-tripped through `Classify` back to the same segments and the same
    trailing-slash form; three-level chains are included, not merely
    two-level — `/category/technology/cloud/aws/`,
    `/category/technology/web/css/`,
    `/category/business/project-management/networking/` — and the deepest, four
    segments under the base
    (`/category/business/project-management/policy-governance/project-management/`),
    carries a leaf slug equal to one of its own ancestors, which is the case a
    walk that matched slugs globally rather than among a term's children would
    get wrong.
  - `TestM9bNestedCategoriesRealDBE2E` (task 10.3) passed — "validated 5 nested
    category archives end-to-end against base `category` and structure
    `/%year%/%monthnum%/%day%/%postname%/`". Five is the file's own
    `realWPCheckCategories` cap, not a shortfall; the census agreed at 28 usable,
    so the `checked > census.Usable` guard held. Over real HTTP for each of
    `/category/personal/alaska/`, `/category/technology/cloud/aws/`,
    `/category/technology/cloud/`, `/category/technology/web/css/` and
    `/category/personal/cycling/`: the canonical nested path `200` with the
    term's name in the rendered heading, the flat `/category/{slug}/` form `301`
    with `Location` byte-equal to the canonical path, the flat form plus
    `?utm_source=…` `301` preserving the query, and following the redirect
    landing `200` at the canonical path rather than at a second redirect.
  - _Read-only throughout._ `storage.New` opened the pool without migrating,
    both tests only read, every assertion was a `GET`, and nothing else was run
    against the database. The repo was mounted `:ro`; the module and build caches
    lived in a podman volume.
  - _The committed tree still skips cleanly._ With `GRIMOIRE_TEST_WP_DSN` unset:
    `gofmt -l .` empty, `go build ./...` clean, `go test ./... -count=1` green
    with both checks reporting `SKIP`.
- [x] 10.6 Update `docs/compatibility.md`: move "No tag, date or author archive
      ...
      _(Req 13.1, 13.5)_
- [x] 10.7 Update the root `README.md` Status section and its M9-remainder
      bullet; both currently name these gaps. _(Req 13.2)_
- [x] 10.8 Update `plans/README.md`: this milestone → Implemented, and tick
...
- [x] 10.9 Confirm no new migration file was added anywhere in the repo
...
- [x] 10.10 Full gate sweep: `gofmt -l .` empty, `go vet ./...`,
