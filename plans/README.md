...
| 09a | [`09-permalinks-canonical-routing`](./09-permalinks-canonical-routing) | ✅ Implemented | Honors `permalink_structure`/`category_base`/`tag_base` from the existing database, serves the core token set (`%postname%`, `%post_id%`, `%year%`, `%monthnum%`, `%day%`) via a new pure `internal/routing` package, and `301`s every non-canonical form — including the flat `/{slug}` path — to one canonical URL per post. Closes the gap where a site on any non-plain permalink structure 404s every URL it has published ([#23](https://github.com/roboweaver/grimoire/issues/23)). An unsupported or unparseable structure degrades to the flat route with a `WARN` at startup rather than refusing to boot. Also extends the existing `render.hierarchy` map with `tag`/`author`/`date` kinds (roadmap 9.F, folded in from M1's template deferral) so the follow-on archive spec adds handlers only. Zero schema changes. **Refines roadmap groups 9.A/9.B/9.F only** — 9.C (tag/date/author archives) and 9.D (nested categories) are deferred to a follow-on spec, which inherits two decisions already resolved in this one: parent category archives include descendants, and nested category routes are canonical with flat `301`ing to them. Two divergences are documented in `docs/compatibility.md`: the configured structure applies to `page` rows as well as `post` rows, where WordPress exempts pages (so a page's canonical URL is dated and `/about` `301`s to it); and `category_base`/`tag_base` were read and resolved but not yet honored by any route, since the category route stayed flat until 9.C/9.D — a divergence M9b closed and removed from `docs/compatibility.md`. The real-WordPress-database checks (`internal/routing/realdb_test.go`, `test/e2e/m9_permalinks_realdb_test.go`) are gated on `GRIMOIRE_TEST_WP_DSN` and have both now passed against a live WordPress database — the `accuweaverllc/scripts` podman stack (MySQL 8.0, prefix `accuweaver`, structure `/%year%/%monthnum%/%day%/%postname%/`, 145 published posts), run from a container on the stack's own network since MySQL is not published to the host; 25 real permalinks validated in the routing check and 5 end-to-end. |
| 09b | [`09.1-archives-nested-categories`](./09.1-archives-nested-categories) | ✅ Implemented | Adds the four archive routes M9a left unserved — nested category, tag, date and author — and makes `category_base`/`tag_base` actually take effect, which M9a resolved but deliberately left inert because the category route stayed flat. Reads `term_taxonomy.parent` into a new `domain.Term.ParentID` (no schema change: the column already exists in every vendor's `0001_init.up.sql` and is populated by WordPress itself), which closes the sharpest gap in the reference database — the `accuweaverllc/scripts` podman stack has **33 categories, 28 of them nested**, every one of which grimoire served as if it were top-level before this milestone. A parent's archive lists and *counts* its descendants' posts through one `EXISTS` semi-join, so a post filed under both a parent and its child appears once and M8's pagination totals stay correct; nested paths are canonical and the flat `/category/{slug}` `301`s to them. The structural core is **one exported classifier**, `routing.Structure.Classify`, with a documented precedence table and a unit test per row: every ambiguous chi pattern registers to a single handler, so chi's silent last-pattern-wins resolution at a parameter node — which M9a discovered and recorded in `router.go` — stops being something the code has to win. Replicates two WordPress behaviors exactly rather than simplifying them: the permalink **front** is prepended to date and author archives always but to category/tag archives only when the corresponding base option is unset (WordPress's `with_front => ! get_option( '{taxonomy}_base' )`, so a base set explicitly to its own default string still drops the front), and date archives move under an extra `date/` segment when `%post_id%` is among the structure's first three tokens, without which `/2024` under a bare `/%post_id%/` structure would make post 2024 unreachable by its own canonical permalink. `UserRepository.ByNicename` diverges deliberately — `ORDER BY ID ASC LIMIT 1` where WordPress issues the same `LIMIT 1` with no `ORDER BY` behind an object cache — because `user_nicename` carries a non-unique index (verified on the live database: `Non_unique = 1`), grimoire's three vendors disagree about unordered row order far more than WordPress's MySQL-only world does, and the cross-vendor contract test needs a stable assertion; `grimoire-cli migrate -check` reports duplicates so an unreachable author is discoverable before serving rather than after. Also carries REST parity (`restTerm.Parent` stops being hard-coded `0`; term and user `link` fields stop advertising URLs that `301` or `404`) and the theme fix that makes pagination links come from the canonical archive path instead of a hard-coded `/category/{{.Term.Slug}}?page=`. Zero schema changes. Two consequences are documented as matching WordPress rather than left to be discovered: a date archive beats a post whose slug is a bare 4-digit number, and an archive base segment beats a post slugged identically to it. Inherits two decisions already settled in M9a (descendant inclusion, nested-is-canonical) rather than relitigating them, and corrects the M9a statements this milestone invalidates in `docs/compatibility.md`, the root `README.md` and `cmd/grimoire-cli/permalinks.go`. Groups **9.C/9.D/9.E** are now ticked in [`wordpress-core-parity-roadmap`](./wordpress-core-parity-roadmap), each with a note naming what shipped, which completes M9. Verified against the live reference database: **33 category terms, 28 nested, all 33 derived paths round-tripping through `Classify`**, and five nested archives checked end-to-end over real HTTP (canonical `200`, flat form `301` with a byte-equal `Location`, query string preserved, one hop only). |
| 08–10 | [`wordpress-core-parity-roadmap`](./wordpress-core-parity-roadmap) | ✅ Implemented (M8, plus **all** of M9 — groups 9.A/9.B/9.F via M9a, groups 9.C/9.D/9.E via M9b) / 📝 Specified (M10 is roadmap-level and requires its own spec before implementation) | UI-parity-first roadmap closing the gaps `docs/compatibility.md`/`docs/wordpress-compatibility-tour.md` document. **M8 Content Browsing Parity** (implementation-ready): public home/category pagination totals + out-of-range 404s; admin post search/status/author filters with 400 invalid-filter handling; media library search/type/date/parent filters, a mutually-exclusive grid/list toggle (replacing today's simultaneous Grid+Table render), and pagination; shared vendor-neutral filter/pagination contracts. No schema change — every new filter/count reuses an existing column (`post_author`, `post_date`, `post_mime_type`, `post_parent`) or existing counter (`PostCounter.CountByStatus`). Supersedes M07's README note that REST media/user writes were deferred to "M8" — this roadmap reassigns them to M10. **M9 Routing & Taxonomy Parity** (implemented across two specs): the `permalink_structure`/`category_base`/`tag_base` options, core permalink tokens + canonical redirects, and template-hierarchy fidelity (groups 9.A/9.B/9.F) were refined into [`09-permalinks-canonical-routing`](./09-permalinks-canonical-routing) and shipped there; tag/date/author archives (9.C), nested categories via the already-populated but unread `term_taxonomy.parent` (9.D) and the cross-vendor `Term.ParentID` coverage plus the nested-category half of the real-database check (9.E) were refined into [`09.1-archives-nested-categories`](./09.1-archives-nested-categories) and shipped there, which also makes the two base options actually take effect. **M10 REST Write & Content Safety Parity** (roadmap-level): a capability-aware write-boundary HTML sanitization policy — introduced for the first time, since no sanitizer exists in the codebase today — landing before REST `/media`/`/users` writes are enabled (closing their `501`s) and before `content.rendered` fidelity improvements (Gutenberg delimiter stripping, responsive images). |
