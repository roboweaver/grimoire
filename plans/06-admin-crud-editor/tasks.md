...
- [ ] 1.11 Write failing unit tests for `content.PostWriteService.Update`'s new concurrency parameter: given a matching `expectedModified`, the update proceeds; given a mismatched one, it returns the new `content.ErrConflict` sentinel without writing; given a zero `expectedModified`, the check is skipped entirely (REST's "no `If-Unmodified-Since`" case); the existing not-found → `ErrForbidden` and capability-check behavior are unchanged for both branches.
- [ ] 1.12 Change `content.PostWriteService.Update`'s signature to accept `expectedModified time.Time` and add `content.ErrConflict`, per `design.md`; update every existing internal caller (none yet outside `internal/web`, which Phase 2/3 will update).
  - _Acceptance:_ All of Phase 1's new/updated unit and contract tests pass; `go build ./...` succeeds repo-wide (temporary compile breaks in `internal/web` from the signature change are expected until Phase 2 lands — do not leave the tree broken between commits within this phase; land 1.11/1.12 together with the minimal `internal/web` call-site fixups needed to keep `go build ./...` green, without yet adding Phase 2's new endpoints). _(Req 3.1–3.3)_
...
- [ ] 2.2 Implement `adminapi_posts.go`'s `adminPostCreate` handler and its shared request-body/detail-shape mapping (Req 4), wired via `PostWriteService.Create` then `PostTermsWriteService.SetPostTerms` per submitted `terms` map key (Req 2.2).
- [ ] 2.3 Write failing HTTP tests for `PUT /admin/api/posts/{id}`: 200 + updated detail shape on a valid body with matching `modified`; 409 with `currentModified` in the body when `modified` is stale (Req 3.2); 404 for a nonexistent ID; 400 for a missing/malformed `modified` field (Req 3.1 — admin API requires it, unlike REST); same validation/authorization matrix as 2.1 for the update-specific capability rules (`edit_others_posts`, `edit_published_posts`, `edit_private_posts`).
- [ ] 2.4 Implement `adminPostUpdate`, threading `modified` into `PostWriteService.Update`'s new `expectedModified` parameter and mapping `content.ErrConflict` to `409`.
- [ ] 2.5 Write failing HTTP tests for `DELETE /admin/api/posts/{id}`: 204 on success; 404 for a nonexistent ID; 403 for missing `delete_posts`/CSRF.
...
- [ ] 2.7 Write failing HTTP tests for `GET /admin/api/terms?taxonomy=category|post_tag`: returns `{id,name,slug,count}` entries sorted by name; 400 for an unrecognized `taxonomy` query value.
...
- [ ] 2.9 Implement `adminapi_terms.go` (`adminTerms`, `adminTermCreate`, `adminTermUpdate`, `adminTermDelete`), wired to `TermWriteService`.
- [ ] 2.10 Add the new routes to `adminroutes.go` per `design.md`'s route table, including the new `requireSessionCSRFJSON` middleware (extracted from the existing inline M4 `requireSessionCSRF` check, same logic, reusable across the posts and terms capability groups).
  - _Acceptance:_ 2.7–2.10's HTTP suite passes; existing M3/M4 admin API HTTP tests (comments, media, menus, read-only posts) still pass unchanged, confirming `requireSessionCSRFJSON`'s extraction didn't change behavior. _(Req 2.1, 2.3–2.5, 8.1–8.4)_
...
- [ ] 3.2 Write failing HTTP tests for `PUT`/`PATCH /wp-json/wp/v2/posts/{id}` and `/pages/{id}`: 200 on a normal update; 409 (`rest_conflict`) when `If-Unmodified-Since` is present and stale; 200 (last-write-wins, no conflict raised) when `If-Unmodified-Since` is **omitted** even though the record has changed since an earlier read (Req 6.5 — the deliberate WP-parity gap vs. the admin API).
...
- [ ] 3.5 Implement the `posts`/`pages` write handlers in `rest_posts.go`, mapping WP-shaped request bodies to `domain.Post`/term-ID assignment (via `PostTermsWriteService`) and parsing `If-Unmodified-Since` as an HTTP-date into `PostWriteService.Update`'s `expectedModified` (zero value when absent), reusing the existing single-item response mapping `handleRESTPostSingle` already has for `GET`.
...
- [ ] 4.3 Implement `components/RichTextEditor.tsx`: `useEditor()` wrapper, Spectrum `ActionButton`/`ToggleButton`/`Picker` toolbar per `design.md`'s component tree, image-insert action opening the existing M4 media-library picker in a `DialogTrigger` and calling `setImage({src})` on selection (Req 7.1, 7.2, 7.3).
