...
- [ ] 3.3 Implement both in `internal/storage/wprepo/repo.go` using an `EXISTS`
      ...
      the listing. Date bounds use the existing `formatTS` helper, matching
      `MediaFilter`'s `After`/`Before`, so one statement serves all three
      vendors. _(Req 3.1, 3.3, 6.4)_
...
- [ ] 4.3 Write a failing `storagetest` contract case for
      `UserRepository.DuplicateNicenames`: reports the pair seeded in 4.1 with
...
- [ ] 4.4 Add `domain.NicenameConflict` and
      `UserRepository.DuplicateNicenames(ctx)` and implement it as
      ...
      three vendors. It is called **only** from `grimoire-cli migrate -check`
      (tasks 9.4–9.5) and from no request path: a per-request duplicate check
      would add a `COUNT` to every author archive hit for a condition that is
      nearly always absent, and the handler must still serve one of the two
      authors either way. _(Req 7.8)_
...
- [ ] 9.2 Update `cmd/grimoire-cli/permalinks.go` to report both bases and the
      notes, and **replace** the comment explaining why the bases are
      deliberately omitted — its stated reason ("no route honors either one yet")
      expires with this milestone, and leaving it would be the same
      misinformation in the opposite direction. _(Req 4.6, 13.3)_
- [ ] 9.3 Confirm `cmd/grimoire/permalinks.go`'s startup `INFO` line still names
      the resolved bases, and log each `Structure.Notes()` entry at `WARN`
      alongside it. _(Req 4.4)_
...
- [ ] 9.5 Wire `UserRepository.DuplicateNicenames` into `reportPreflight` in
      `cmd/grimoire-cli/main.go`, alongside the existing `reportPermalinks` call
