...
- [x] 1.1 Verify prerequisites and select content.
...
- [x] 1.2 Check the shared environment safely.
...
- [x] 1.3 Start both local sites and establish admin sessions.
...
- [x] 1.4 Normalize and verify all seven surfaces.
...
- [x] 1.5 Capture the seven WordPress/Grimoire pairs.
  ...
  - _Note:_ The Grimoire media library route (`?parentId=`) turned out to be
    ignored entirely by the frontend (a genuine, previously-undocumented
    product limitation — the backend supports the filter, the admin UI never
    reads it). The operator approved an alternate DOM-based approach: remove
    all non-target gallery cards/rows and redact identity in-DOM before
    writing the file, same as the posts-list capture. See the "Media
    library" section of `docs/wordpress-compatibility-tour.md` for the
    accurate, corrected description of this limit.
- [x] 1.6 Inspect every image immediately.
...
- [x] 2.1 Create `docs/wordpress-compatibility-tour.md`.
  - _Acceptance:_ Exactly seven ordered sections use two-column image tables and
    ...
    _(Req 1, 2, 6)_
- [x] 2.2 Refresh `docs/compatibility.md` to current M7 behavior.
  - _Acceptance:_ Stale M1/read-only claims are removed; current coverage, trust
    ...
    _(Req 7)_
- [x] 2.3 Add README links and cross-links.
