...
and `plans/README.md`; the only additional changed file permitted is
`plans/readme-public-release/tasks.md` for checkbox tracking. No application
behavior, generated brand asset, badge, or dependency change is permitted.
...
- [x] 1.1 Update the H1 to display `assets/icons/icon-64.png` inline at
...
- [x] 1.2 Rewrite the opening in present tense and add the early current-state
...
- [x] 1.3 Update Goals and rename `Running (M1)` to `Running`; describe the
...
- [x] 1.4 Convert M1-M7 into seven `✅` linked entries using the exact plan
...
- [x] 1.5 Replace the open-ended licensing note with an Apache-2.0 statement and
...
- [x] 2.1 Download the canonical Apache License 2.0 text from
  ...
  - _Acceptance:_ The canonical-license comparison command in `design.md` exits
    zero with no output.
  - _Requirements:_ 5.1.
- [x] 2.2 In `plans/README.md`, change rows 01, 02, 02.1, 02.2, and 03 to
...
- [x] 2.3 Move License from Open decisions into a new Resolved decisions section
...
- [x] 3.1 Validate all changed local Markdown links and the icon path.
  ...
  - _Acceptance:_ Every command exits zero; no changed local link is unchecked
    or broken.
  - _Requirements:_ 7.1.
- [x] 3.2 Validate the canonical Apache-2.0 text.
...
- [x] 3.3 Validate README/plans consistency and scan for stale status phrases.
...
- [x] 3.4 Validate scope and Markdown hygiene.
  - _Action:_ Run `git diff --check`, `git status --short`, and
    `git diff --name-only`.
  - _Acceptance:_ There are no whitespace errors and exactly `README.md`,
    `LICENSE`, `plans/README.md`, and `plans/readme-public-release/tasks.md`
    are changed; no application, asset, or dependency file appears.
  - _Requirements:_ 7.5.
- [x] 3.5 Commit the later implementation as one documentation change.
  - _Action:_ Stage `README.md`, `LICENSE`, `plans/README.md`, and
    `plans/readme-public-release/tasks.md`, then commit with the approved
    implementation message and the standard
  ...
  - _Acceptance:_ `git status --short` is clean after the commit and the commit
    contains exactly `README.md`, `LICENSE`, `plans/README.md`, and
    `plans/readme-public-release/tasks.md`.
